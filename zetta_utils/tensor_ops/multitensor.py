...
class MultiTensorOp(Protocol[P]):
    ...
    def __call__(self, data1: TensorTypeVar, data2: TensorTypeVar, *args: P.args, **k: P.kwargs) -> TensorTypeVar:
        ...
...
def skip_on_empty_datas(fn: OpT) -> OpT:
    ...
    def wrapped(data1: TensorTypeVar, data2:TensorTypeVar, *args: P.args, **kwargs: P.kwargs) -> TensorTypeVar:
...
@skip_on_empty_datas
def compute_pixel_error(data1: TensorTypeVar, data2:TensorTypeVar, erosion: int = 5, **kwargs) -> TensorTypeVar:
    """
...
@skip_on_empty_datas
def erode_combine(data1: TensorTypeVar, data2: TensorTypeVar, erosion: int = 5, **kwargs) -> TensorTypeVar:
    """
    ...
    result = torch.where(torch.logical_and(mask1, mask2), (0.5 * data1 + 0.5 * data2).to(data1.dtype), torch.zeros_like(data1))
